#
# $Header: svn://svn/SWM/trunk/web/ClearanceObj.pm 10771 2014-02-21 00:20:57Z cgao $
#

package ClearanceObj;

#TO DO:
### I HAVE ADDED ALL Clearances.pm subs at around line 340.  PLEASE CLEAN UP
## DO finaliseClearance
...
#--Move Updatepath to the ClearancePathObj.  It will then loop through all paths in self->{'Paths'} and call an updateNode
#--All the form stuff for clearanceCancel should still be in a Clearances.pm::cancelClearance similar call which THEN calls this object

use lib ".", "..", "comp/";
use AuditLog;
use ClearancePathObj;
...
sub load	{
  ...
  if($DBI::err) {
    ...
    $self->{'DBData'}=$q->fetchrow_hashref();
		$self->{'ID'} = $self->{'DBData'}{'intClearanceID'};
  }

	if ($args{'loadPaths'})	{
		print "ABOUT TO LOAD FOR $self->{'ID'}\n";
		#$self->{'Paths'} = new ClearancePathObj('clearanceID' => $self->{'ID'}, 'db' => $self->{'db'});
		#$self->{'Paths'}->load();
		$self->{'Paths'} = $self->getPathNodes();
	}
...
sub getClearancePath	{
	my $self = shift;

	#$self->{'Paths'} = new ClearancePathObj(clearanceID => $self->{'ID'}, db => $self->{'db'}) if ! $self->{'Paths'};
	#check if loaded
...
sub getValue  {
  my $self = shift;
	my($field)=@_;
	return $self->{$field} || $self->{'DBData'}{$field};
}
...
### The club can reopen it at any stage. See clearanceReopen(). 
  ...
  my ($Data) = @_;
	...
	my $clearanceID = $self->getValue('intClearanceID') || 0;
  my $clubID = $Data->{'clientValues'}{'clubID'} || 0;
  return qq[Clearance Unabled to be cancelled] if (! $clubID or ! $clearanceID);

	my $intPermitType= $self->getValue('intPermitType');
	my $endPermit= $self->getValue('EndPermit');

	my %updateArgs=();
	$updateArgs{'intClearanceStatus'} = $Defs::CLR_STATUS_CANCELLED;
	my %whereArgs=();
	$whereArgs{'intClearanceStatus'} = $Defs::CLR_STATUS_PENDING;

	$self->update(
		\%updateArgs,
		\%whereArgs
	); ## Updates intClearanceStatus

	#foreach my $path (@{$self->{'Paths'}})	{
	#	$path->updateNode(
	#		\%updateArgs, 
	#		\%whereArgs
	#	); ## Needs to set clearanceStatus ! Where = PENDING
	#}


	foreach my $node (@{$self->{'Paths'}})	{
		$node->update(
			\%updateArgs, 
			\%whereArgs
		); ## Needs to set clearanceStatus ! Where = PENDING
  ...
  if ($Data->{'SystemConfig'}{'clrAllowPermits'} and $endPermit and $intPermitType) {
    $self->clearanceReversePermit($Data);
  }
}
 ...
 my ($where, $whereArgs_ref) = @_;
  if($self->ID()) {
    my $st=qq[
      ...
      SET
				intClearanceStatus = ?
      WHERE intClearanceID = ?
				$where
    ];
		print STDERR $st . "$self->{'DBData'}{'intClearanceStatus'} | " . $self->ID() . "\n";
    my $q = $self->{'db'}->prepare($st);
    $q->execute(
      ...
      $self->ID(),
		...
		my $st = qq[
			INSERT INTO tblClearance (intClearanceStatus)
			VALUES (?)
		];
		my $q = $self->{'db'}->prepare($st);
    $q->execute(
      $self->getValue('intClearanceStatus'),
		);
      #$self->{'DBData'}{'intClearanceStatus'},
		$self->{'ID'}=$q->{'mysql_insertid'};
    $self->{'DBData'}{'intClearanceID'} = $q->{'mysql_insertid'};
	}
}
...
sub getPathNodes {
  ...
  $qry->execute($self->{'ID'});
  my $count=0;
  print $st . " FOR " . $self->{'ID'} . "\n";
  while (my $dref = $qry->fetchrow_hashref()) {
  print "CP FROM DB: $dref->{'intClearancePathID'} FOR " . $self->{'ID'} . "\n";
    my $node_obj= new ClearanceNodeObj('clearancePathID'=>$dref->{'intClearancePathID'}, 'clearanceID'=>$self->{'ID'}, 'db'=>$self->{'db'});
    $node_obj->load();
...
### At the bottom of this function is the ability to add a manual clearance history record.  For example, if they came from overseas.  These manual records don't have approval paths, but rather, are for historical purposes.
	...
	$body .= qq[
		...
		<th>Ref. No.</th>
		<th>Date</th>
		<th>From League (Club)</th>
		<th>To League (Club)</th>
		<th>Status</th>
		<th>Type</th>
		</tr>
	];
	...
	while (my $dref=$query->fetchrow_hashref())	{
		...
		my $clearanceID= qq[<a href="$Data->{'target'}?client=$client&amp;cID=$dref->{intClearanceID}&amp;a=CL_view">$dref->{intClearanceID}</a>];
		$clearanceID= qq[<a href="$Data->{'target'}?client=$client&amp;clrID=$dref->{intClearanceID}&amp;a=CL_editmanual">$dref->{intClearanceID}</a>] if ($dref->{intCreatedFrom} == $Defs::CLR_TYPE_MANUAL and $Data->{'clientValues'}{'authLevel'} >= $Defs::LEVEL_ASSOC);
...
########
		...
		$body .=qq[
			<tr>
				<td>$clearanceID</td>
				<td>$dref->{dtApplied}</td>
				<td>$dref->{SourceAssocName} ($dref->{SourceClubName})</td>
				<td>$dref->{DestinationAssocName} ($dref->{DestinationClubName})</td>
				<td>$status</td>
				<td>$clearance_type</td>
			</tr>
		];
	}
	$body .= qq[</table>];
	...
	if ($Data->{'SystemConfig'}{'clrFilterMSPlayers'})	{
		...
		$st = qq[
			...
			WHERE M.intRealmID = $Data->{'Realm'}
				...
				$CLRD_OUT_WHERE
			GROUP BY M.intMemberID, A.intAssocID, C.intClubID
			ORDER BY CLR_DATE DESC, M.strSurname, M.strFirstname, M.dtDOB
		];
		...
		Clearance => {
			fields => {
				...
				BTNTEXT=> {
                           ...
                           value => qq[<div class="HTbuttons"> <input type="submit" name="subbut2" value="$update_labelClr" class="HF_submit" id="HFsubbut2"> </div>],
                           type  => 'textvalue',
                       },
                       intPermitType	=> {
                                            label => "Permit Type",
...
print STDERR "ACA2 MEM: $memberID PT$permitType\n";
    
print STDERR "ACA3 MEM: $memberID PT$permitType\n";
...
print STDERR "ACA4 MEM: $memberID PT$permitType\n";
...
print STDERR "ACA5 MEM: $memberID PT$permitType\n";
    ...
    if ($Data->{'SystemConfig'}{'clrCheckRunningPermits'})	{
        my $permitCheck_st = qq[
			SELECT
				C1.strName as DestinationClubName, C2.strName as SourceClubName, A1.strName  as DestinationAssocName, A1.intAssocID as DestinationAssocID, A2.strName as SourceAssocName, A2.intAssocID as SourceAssocID, DATE_FORMAT(dtPermitFrom,'%d/%m/%Y') AS PermitFrom,  DATE_FORMAT(dtPermitTo,'%d/%m/%Y') AS PermitTo, A1.strEmail as DestinationAssocEmail, A1.strPhone as DestinationAssocPh, A1.strContact as DestinationAssocContact,  A2.strEmail as SourceAssocEmail, A2.strPhone as SourceAssocPh, A2.strContact as SourceAssocContact, C.intSourceClubID, C.intDestinationClubID
			FROM
				tblClearance as C
				LEFT JOIN tblClub as C1 ON (C1.intClubID = C.intDestinationClubID)
				LEFT JOIN tblClub as C2 ON (C2.intClubID = C.intSourceClubID)
        ...
        while (my $dref = $query->fetchrow_hashref())	{
            ...
            # If the player is permitted to an association that allows multiple permits - that's OK
						if ($params->{'sourceClubID'} != $dref->{'intSourceClubID'} or $params->{'destinationClubID'} != $dref->{'intDestinationClubID'})	{
...
print STDERR "ACA6 MEM: $memberID PT$permitType\n";
    
	if ($Data->{'SystemConfig'}{'clrAllowPermits'} and $permitType and $permitType == 1)	{
        #return (1,'');
	}
print STDERR "ACA7 MEM: $memberID PT$permitType\n";
	...
	my $st = qq[
			SELECT
				C.intClearanceID, C1.strName as DestinationClubName, C2.strName as SourceClubName, A1.strName  as DestinationAssocName, A1.intAssocID as DestinationAssocID, A2.strName as SourceAssocName, DATE_FORMAT(dtApplied,'%d/%m/%Y') AS AppliedDate, A1.strEmail as DestinationAssocEmail, A1.strPhone as DestinationAssocPh, A1.strContact as DestinationAssocContact,  A2.strEmail as SourceAssocEmail, A2.strPhone as SourceAssocPh, A2.strContact as SourceAssocContact
			FROM
				tblClearance as C
				LEFT JOIN tblClub as C1 ON (C1.intClubID = C.intDestinationClubID)
				LEFT JOIN tblClub as C2 ON (C2.intClubID = C.intSourceClubID)
	...
	my $existingClearance=0;
     ...
     while (my $dref = $query->fetchrow_hashref())	{
         my $multiple_permit_assoc = association_allows_multiple_permits($db, $dref->{'DestinationAssocID'});
         if ($multiple_permit_assoc) {
print STDERR "ACA8 MEM: $memberID PT$permitType - MPA: $multiple_permit_assoc\n";
             next;
         }
...
print STDERR "ACA9 MEM: $memberID PT$permitType - EXISTING COUNT: $existingClearance | $error_text\n";
#	$error_text .= qq[
