...
@ISA = qw(Exporter);
@EXPORT =
  qw(checkAutoConfirms handleClearances clearanceHistory sendCLREmail finaliseClearance insertSelfTransfer getClrTaskCount);
@EXPORT_OK =
  qw(checkAutoConfirms handleClearances clearanceHistory sendCLREmail finaliseClearance insertSelfTransfer getClrTaskCount);
use lib '.', '..', '../..', "../comp", '../RegoForm', "../dashboard",
  "../RegoFormBuilder", '../PaymentSplit', "../user";
...
sub insertSelfTransfer {
	...
	my ( $clrAddStatus, undef ) =
	  postClearanceAdd( $clrID, \%params, 'ADD', $Data, $Data->{'db'} );
}
...
sub handleClearances {
		...
		&& $Data->{'clientValues'}{'authLevel'} < $Defs::LEVEL_NATIONAL )
	{
		my $resultHTML = qq[
		  ...
		  . $lang->txt('Entity Not Allowed To View Transfer Details')
		  . q[</div>
    		];
	...
	###### end check
	return ( createClearance( $action, $Data ), $txt_RequestCLR )
	  if $action eq 'CL_createnew';
	return ( clearancePathDetails( $Data, $clearanceID, $clearancePathID ),
		$lang->txt('Transfer Status Selection') )
	  if $action eq 'CL_details';
	return ( listClearances($Data), $txt_RequestCLR ) if $action eq 'CL_list';
	return ( listOfflineClearances($Data), $txt_RequestCLR )
	  if $action eq 'CL_offlist';
	return ( clearanceView( $Data, $clearanceID ),
		$lang->txt('Clearance Details') )
	  if $action eq 'CL_view';
	return ( clearanceCancel( $Data, $clearanceID ),
		$lang->txt('Cancel Transfer') )
	  if $action eq 'CL_cancel';
	return ( clearanceReopen( $Data, $clearanceID ),
		$lang->txt('Reopen Transfer') )
	  if $action eq 'CL_reopen';
	return ( clearanceAddManual($Data), $lang->txt("Add Manual Transfer") )
	  if $action eq 'CL_addmanual';
	return ( clearanceAddManual($Data), $lang->txt("Edit Manual Transfer") )
	  if $action eq 'CL_editmanual';
}
...
### The club can reopen it at any stage. See clearanceReopen().
	...
	return $lang->txt("Transfer unable to be cancelled")
	  if ( !$clubID or !$clearanceID );
	...
	$qry->execute or query_error($st);
	my ( $intPersonID, $intDestinationEntityID, $intSourceEntityID ) =
	  $qry->fetchrow_array();
	...
	return
	    qq[<p>]
	  . $lang->txt("Transfer Cancelled")
	  . qq[</p><br> <a href="$Data->{'target'}?a=CL_list&amp;client=$client">]
	  . $lang->txt('Return to Transfer Listing')
	  . qq[</a>];
}
...
### See the above function clearanceCancel() for how the destination club actually cancels it.
	...
	my $clubID = getID( $Data->{'clientValues'} );    #{'clubID'} || 0;
	my $lang   = $Data->{'lang'};
	return $lang->txt('Transfer unable to be Reopened')
	  if ( !$clubID or !$clearanceID );
	my $client = setClient( $Data->{'clientValues'} ) || '';
...
AND  C.intClearanceStatus = $Defs::CLR_STATUS_PENDING
	...
	while ( my $dref = $query->fetchrow_hashref() ) {
		...
		if ( $dref->{SourceEntityID} > 0 ) {
			my $source_contactObj = ContactsObj->getList(
				dbh           => $db,
				clubid        => $dref->{SourceEntityID},
				getclearances => 1
			) || [];
			my $source_contactObjP = ContactsObj->getList(
				dbh        => $db,
				clubid     => $dref->{SourceEntityID},
				getprimary => 1
			) || [];
			if ( scalar(@$source_contactObj) > 0 ) {
				$source_contact_name =
qq[@$source_contactObj[0]->{strContactFirstname} @$source_contactObj[0]->{strContactSurname}];
				$source_contact_email =
				  @$source_contactObj[0]->{strContactEmail};
			}
			elsif ( scalar(@$source_contactObjP) > 0 ) {
				$source_contact_name =
qq[@$source_contactObjP[0]->{strContactFirstname} @$source_contactObjP[0]->{strContactSurname}];
				$source_contact_email =
				  @$source_contactObjP[0]->{strContactEmail};
			}
		}
		if ( $dref->{DestinationEntityID} > 0 ) {
			my $destination_contactObj = ContactsObj->getList(
				dbh           => $db,
				clubid        => $dref->{DestinationEntityID},
				getclearances => 1
			);
			my $destination_contactObjP = ContactsObj->getList(
				dbh        => $db,
				clubid     => $dref->{DestinationEntityID},
				getprimary => 1
			);
...
qq[@$destination_contactObj[0]->{strContactFirstname} @$destination_contactObj[0]->{strContactSurname}];
				$destination_contact_email =
				  @$destination_contactObj[0]->{strContactEmail};
			}
			elsif ( scalar(@$destination_contactObjP) > 0 ) {
				$destination_contact_name =
qq[@$destination_contactObjP[0]->{strContactFirstname} @$destination_contactObjP[0]->{strContactSurname}];
				$destination_contact_email =
				  @$destination_contactObjP[0]->{strContactEmail};
			}
		}
		...
		return qq[
			<div class="warningmsg">]
		  . $lang->txt(
'The selected member is already involved in a pending clearance.  Unable to continue until the below transaction is finalised.'
		  )
		  . qq[</div>
			<p>
					<b>] . $lang->txt('Date Requested') . qq[:</b> $dref->{AppliedDate}<br>
					<b>]
		  . $lang->txt('Requested From')
		  . qq[:</b> $dref->{SourceEntityName} ($source_contact_name)<br>
					] . $lang->txt('Contact') . qq[: $dref->{SourceContact}<br>
					]
		  . $lang->txt('Phone')
		  . qq[: $dref->{SourcePh}&nbsp;&nbsp;]
		  . $lang->txt('Email')
		  . qq[:  $dref->{SourceEmail}<br>
					<b>]
		  . $lang->txt('Request To')
		  . qq[:</b> $dref->{DestinationEntityName}<br>
					] . $lang->txt('Contact') . qq[: $dref->{DestinationContact}<br>
					]
		  . $lang->txt('Phone')
		  . qq[: $dref->{DestinationPh}&nbsp;&nbsp;]
		  . $lang->txt('Email')
		  . qq[: $dref->{DestinationEmail}<br>
				</p>
			];
	...
	auditLog( $clearanceID, $Data,
		$lang->txt('Reopen') . $lang->txt('Transfer') );
	return
	    qq[<p>]
	  . $lang->txt('Transfer Reopened')
	  . qq[</p><br> <a href="$Data->{'target'}?a=CL_list&amp;client=$client">]
	  . $lang->txt('Return to Transfer Listing')
	  . qq[</a>];
}
...
### At the bottom of this function is the ability to add a manual clearance history record.  For example, if they came from overseas.  These manual records don't have approval paths, but rather, are for historical purposes.
	...
	while ( my $dref = $query->fetchrow_hashref() ) {
		...
		my $priority =
		  $Defs::clearance_priority{ $dref->{intClearancePriority} };
		$dref->{SourceEntityName} ||= $dref->{strSourceEntityName} || '';
		$dref->{DestinationEntityName} ||= $dref->{strDestinationEntityName}
		  || '';
		my $selectLink =
"$Data->{'target'}?client=$client&amp;cID=$dref->{intClearanceID}&amp;a=CL_view";
		$selectLink =
"$Data->{'target'}?client=$client&amp;clrID=$dref->{intClearanceID}&amp;a=CL_editmanual"
		  if (  $dref->{intCreatedFrom} == $Defs::CLR_TYPE_MANUAL
			and $Data->{'clientValues'}{'authLevel'} >= $Defs::LEVEL_ASSOC );
...
## TC ## HACKED IN COS TOO MANY BAFF CHANGES ON DEVEL | CHANGE IS CHECKED IN ON DEVEL
	...
	$body =
	    qq[<div class="warningmsg">]
	  . $lang->txt('No Transfer History found')
	  . qq[</div>]
	  if !$count;

	my $addManual =
	    qq[<a href="$Data->{'target'}?client=$client&amp;a=CL_addmanual">]
	  . $lang->txt('Add manual Transfer History')
	  . qq[</a>];
...
## The function works out if the level viewing it is the destination club (who requested the clearance) and if so allows the clearance notes field to be updated.
	...
	if (   $dref->{intCreatedFrom} == $Defs::CLR_TYPE_MANUAL
		or $dref->{intSourceEntityID} == getID( $Data->{'clientValues'} ) )
	{    #{'clubID'})	{
		$edit   = 1;
		$id     = $cID;
		$option = 'edit';
	...
	my $readonly = 1;
	$readonly = 0
	  if ( getID( $Data->{'clientValues'} ) == $dref->{intDestinationEntityID} )
	  ;    #clubID
	$dref->{SourceEntityName}      ||= $dref->{strSourceEntityName}      || '';
	$dref->{DestinationEntityName} ||= $dref->{strDestinationEntityName} || '';
	my $update_label = $Data->{'SystemConfig'}{'txtUpdateLabel_CLR'}
	  || 'Update Clearance';
	my $intReasonForClearanceID =
	  ( $Data->{'SystemConfig'}{'clrHide_intReasonForClearanceID'} == 1 )
	  ? '1'
	  : '0';
	my $strReasonForClearance =
	  ( $Data->{'SystemConfig'}{'clrHide_strReasonForClearance'} == 1 )
	  ? '1'
	  : '0';
	my $strFilingNumber =
	  ( $Data->{'SystemConfig'}{'clrHide_strFilingNumber'} == 1 ) ? '1' : '0';
	my $intClearancePriority =
	  ( $Data->{'SystemConfig'}{'clrHide_intClearancePriority'} == 1 )
	  ? '1'
	  : '0';
	my $strReason =
	  ( $Data->{'SystemConfig'}{'clrHide_strReason'} == 1 ) ? '1' : '0';
		...
		CLR => {
			fields => {
				...
				ClearanceStatus => {
					...
					value =>
qq[<b>$Defs::clearance_status{$dref->{intClearanceStatus}}</b>],
					type     => 'text',
					readonly => '1',
				},
				ClearancePriority => {
					label => $lang->txt('Transfer Priority'),
					value => $Defs::clearance_priority{ $dref
			...
			order => [
				qw(ClearanceID dtApplied NatNum MemberName dtDOB strState SourceEntityName DestinationEntityName ClearanceStatus ClearanceReason intReasonForClearanceID strReason strReasonForClearance)
			],
			options => {
				labelsuffix         => ':',
				hideblank           => 1,
				...
				stopAfterAction     => 1,
				updateOKtext        => qq[<div class="OKmsg">]
				  . $lang->txt('Record updated successfully')
				  . qq[</div> <br>                                        <a href="$Data->{'target'}?client=$client&amp;a=CL_view&amp;cID=$cID">]
				  . $lang->txt('Return to Transfer')
				  . qq[</a>],
			},
	...
	( $resultHTML, undef ) =
	  handleHTMLForm( $FieldDefs{'CLR'}, undef, $option, '', $db );

	if ( $dref->{intCreatedFrom} == 0 ) {
		$resultHTML .=
qq[<a href="$Data->{'target'}?client=$client&amp;cID=$cID&amp;a=CL_cancel">]
		  . $lang->txt('Cancel Transfer')
		  . qq[</a>]
		  if (
			$dref->{intDestinationEntityID} == getID( $Data->{'clientValues'} )
			and $dref->{intClearanceStatus} != $Defs::CLR_STATUS_CANCELLED
			and $dref->{intClearanceStatus} != $Defs::CLR_STATUS_APPROVED );
		$resultHTML .=
qq[<a href="$Data->{'target'}?client=$client&amp;cID=$cID&amp;a=CL_reopen">]
		  . $lang->txt('Reopen Cancelled Transfer')
		  . qq[</a>]
		  if (
			$dref->{intDestinationEntityID} == getID( $Data->{'clientValues'} )
			and $dref->{intClearanceStatus} == $Defs::CLR_STATUS_CANCELLED );
		$resultHTML .=
		  showPathDetails( $Data, $cID, $dref->{intClearanceStatus} );

		#DISPLAY DOCUMENT LIST
		$resultHTML .= showTransferDocuments( $Data, $cID );
	}
	else {
		$resultHTML .=
		  qq[<br><div class="warningmsg">]
		  . $lang->txt(
'No path details can be shown as this Transfer was created offline or it is a Manual Transfer History record'
		  ) . qq[</div>];
	}

	if ( $option eq 'display' ) {
		$resultHTML .=
		    qq[<br><a href="$target?a=CL_list&amp;client=$client">]
		  . $lang->txt('Return to Transfer Listing')
		  . qq[</a> ];
	}
...
### If the destination club (who requested clearance) have cancelled the clearance, this function has the "REOPEN" option set against the status column.
	...
	$body .= qq[
	  ...
	  . $lang->txt('Transfer Approval Details')
	  . qq[</div>
...
####
	my $intID = 0;
	$intID = $Data->{'clientValues'}{'clubID'}
	  if ( $Data->{'clientValues'}{'currentLevel'} == $Defs::LEVEL_CLUB );
	$intID = $Data->{'clientValues'}{'zoneID'}
	  if ( $Data->{'clientValues'}{'currentLevel'} == $Defs::LEVEL_ZONE );
	$intID = $Data->{'clientValues'}{'regionID'}
	  if ( $Data->{'clientValues'}{'currentLevel'} == $Defs::LEVEL_REGION );
	$intID = $Data->{'clientValues'}{'stateID'}
	  if ( $Data->{'clientValues'}{'currentLevel'} == $Defs::LEVEL_STATE );
	$intID = $Data->{'clientValues'}{'natID'}
	  if ( $Data->{'clientValues'}{'currentLevel'} == $Defs::LEVEL_NATIONAL );
	my $intTypeID = $Data->{'clientValues'}{'currentLevel'};

	my @rowdata = ();
	while ( my $dref = $query->fetchrow_hashref() ) {
		my ( $pathnode, undef, undef ) =
		  getNodeDetails( $db, $dref->{intTableType}, $dref->{intTypeID},
			$dref->{intID} );
		next if !$pathnode;
		my $status =
		  ( $denied == 1 )
		  ? '-'
		  : $Defs::clearance_status{ $dref->{intClearanceStatus} };
		$denied = 1 if $dref->{intClearanceStatus} == $Defs::CLR_STATUS_DENIED;
		$status =
qq[<a href="$Data->{'target'}?client=$client&amp;a=CL_details&amp;cID=$dref->{intClearanceID}&amp;cpID=$dref->{intClearancePathID}">$status</a>]
		  if (  $status ne '-'
			and $dref->{intClearanceStatus} != $Defs::CLR_STATUS_PENDING
			and $dref->{intTypeID} == $intTypeID
			and $dref->{intID} == $intID
			and !$Data->{'ReadOnlyLogin'} );

		$status =
		  qq[<span style="font-weight:bold;color:green;">$status</style>]
		  if $dref->{intClearanceStatus} == $Defs::CLR_STATUS_APPROVED;
		$status = qq[<span style="font-weight:bold;color:red;">$status</style>]
		  if $dref->{intClearanceStatus} == $Defs::CLR_STATUS_DENIED;
		$status .=
qq[&nbsp;&nbsp;<a href="$Data->{'target'}?client=$client&amp;a=CL_details&amp;cID=$dref->{intClearanceID}&amp;cpID=$dref->{intClearancePathID}">&nbsp;(--]
		  . $lang->txt('Reopen Transfer')
		  . qq[--)</a>]
		  if (  $dref->{intClearanceStatus} == $Defs::CLR_STATUS_DENIED
			and $dref->{intTypeID} == $intTypeID
			and $dref->{intID} == $intID );
		$status = $lang->txt('Cancelled')
		  if (  $clearanceStatus == $Defs::CLR_STATUS_CANCELLED
			and $dref->{intClearanceStatus} == $Defs::CLR_STATUS_PENDING );
...
### If the level at view the path details, they are given the option of editing the notes.
	...
	$extrafields_readonly = 1
	  if ( $dref->{intTypeID} != $intTypeID
		or $dref->{intID} != $intID
		or $dref->{intCurrentPathID} < $cpID );
	$readonly = 1 if ( $dref->{intCurrentPathID} != $cpID );
	$readonly = 1
	  if ( $dref->{intClearanceStatus} == $Defs::CLR_STATUS_APPROVED );    #BAFF
	...
	$option = 'display'
	  if (  $dref->{intClearanceStatus} == $Defs::CLR_STATUS_APPROVED
		and $Data->{'SystemConfig'}{'NoEditClearanceOnceApproved'} );
	$option = 'display'
	  if (  $Data->{'ReadOnlyLogin'}
		and $Data->{'clientValues'}{'authLevel'} >= $Defs::LEVEL_ASSOC ); ##BAFF
	my ( $DefCodes, $DefCodesOrder ) = getDefCodes(
		dbh        => $Data->{'db'},
		realmID    => $Data->{'Realm'},
		subRealmID => $Data->{'RealmSubType'},
...
#my $update_label = $Data->{'SystemConfig'}{'txtUpdateLabel_CLR'} || 'Update Clearance';
		...
		CLR => {
			fields => {
				...
				strApprovedBy => {
					...
					readonly => $extrafields_readonly,
					compulsory =>
					  $Data->{'SystemConfig'}{'ClrApprovedBy_NotCompulsory'}
					? 0
					: 1,
				},
				strPathFilingNumber => {
			...
			order => [
				qw(intClearanceID MemberName dtDOB strState SourceEntityName DestinationEntityName intClearanceStatus strApprovedBy intDenialReasonID strPathNotes)
			],
			options => {
				labelsuffix    => ':',
				hideblank      => 1,
				...
				auditAddParams => [ $Data, 'Add', $lang->txt('Transfer Path') ],
				auditEditParams =>
				  [ $cpID, $Data, 'Update', $lang->txt('Transfer Path') ],

				afterupdateFunction => \&updateClearance,
				afterupdateParams =>
				  [ $option, $Data, $Data->{'db'}, $cID, $cpID ],
				stopAfterAction => 1,
				updateOKtext    => qq[<div class="OKmsg">]
				  . $lang->txt('Record updated successfully')
				  . qq[</div> <br>
					<a href="$Data->{'target'}?client=$client&amp;a=CL_view&amp;cID=$cID&amp;cpID=$cpID">]
				  . $lang->txt('Return to Transfer Details') . qq[</a>
				],
				addOKtext => qq[
					<div class="OKmsg">]
				  . $lang->txt('Record updated successfully')
				  . qq[</div> <br>
					<a href="$Data->{'target'}?client=$client&amp;a=CL_details">]
				  . $lang->txt('Return to Transfer Details') . qq[</a>
				],
			},
			sections => [ [ 'main', $lang->txt('Details') ], ],
	...
	$FieldDefs{'CLR'}{'fields'}{'intDenialReasonID'}{'compulsory'} = 1
	  if ( $Data->{'SystemConfig'}{'clrDenialReason_compulsory'}
		and param('d_intClearanceStatus') == $Defs::CLR_STATUS_DENIED );
	( $resultHTML, undef ) =
	  handleHTMLForm( $FieldDefs{'CLR'}, undef, $option, '', $db );
	$resultHTML = destinationEntityText($Data) . $resultHTML
	  if ( $dref->{intDestinationEntityID} == $dref->{intID} );
	...
	if ( $option eq 'display' ) {
		$resultHTML .=
qq[ <a href="$target?a=CL_details&amp;tID=$dref->{'intTransactionID'}&amp;client=$client">]
		  . $lang->txt('Edit Details')
		  . qq[</a> ]
		  if !$Data->{'ReadOnlyLogin'};
	}
	...
	$resultHTML =
	  qq[<div>]
	  . $lang->txt(
		'This member does not have any Transaction information to display')
	  . qq[</div>
		] if !ref $dref;
...
sub memberLink {
	...
	if (    $dref->{intClearanceID}
		and $dref->{intPersonID}
		and $dref->{intClearanceStatus} == $Defs::CLR_STATUS_APPROVED )
	{
		...
		return
		    qq[ <div class="OKmsg">]
		  . $lang->txt('The transfer has now been finalised')
		  . qq[</div><br><a href="$Data->{'target'}?client=$tempClient&amp;a=P_HOME">]
		  . $lang->txt('click here to display persons record')
		  . qq[</a>];
...
### PURPOSE: This function returns text that is displayed at the top of the clearance approval record when its the destination clubs turn to decide whether they actually want to member.  At this stage, all other levels would have approved the clearance.
	...
	return '';
	my $body = $lang->txt(
'By approving this transfer, you will receive the person.<br>You also agree to pay any Fees incurred by the transferring of this player'
	);
...
### Also, if the next path level is invisible (ie: an invisible zone level), then its auto confirmed.
	...
	my $qry_path_update = $db->prepare($st_path_update)
	  or query_error($st_path_update);
	...
	while ( my $dref = $query->fetchrow_hashref() ) {
		...
		my $intAutoApproval =
		  getClearanceSettings( $Data, $dref->{intID}, $dref->{intTypeID},
			$dref->{dtDOB}, $dref->{intDirection} );
		if (    $dref->{intNodeID}
			and $dref->{intStatusID} eq '0'
			and $intAutoApproval == 0 )
		{
			$intAutoApproval = $Defs::CLR_AUTO_APPROVE;
		}
		if ( $intAutoApproval == $Defs::CLR_AUTO_APPROVE ) {
			my $clearancePathStatus = $intAutoApproval;
			$qry_path_update->execute( $clearancePathStatus,
				$dref->{intClearancePathID} )
			  or query_error($st_path_update);
			$currentPathID = $dref->{intClearancePathID};
		}
			...
			$clearanceStatus = 2;
			$currentPathID   = $dref->{intClearancePathID};
			$qry_path_update->execute( $clearancePathStatus, 0,
				$dref->{intClearancePathID} )
			  or query_error($st_path_update);
		}
		else {
	...
	if ( $currentPathID or $clearanceStatus ) {
		...
		my $qry_clr_update = $db->prepare($st_clr_update)
		  or query_error($st_clr_update);
...
### PURPOSE: This function returns the clearance settings for the level and members DOB
	...
	my ( $intClearanceSettingID, $intAutoApproval, $intRuleDirection ) =
	  $query->fetchrow_array();
	$intRuleDirection      ||= $Defs::CLR_BOTH;
	$intClearanceSettingID ||= 0;
	$intAutoApproval       ||= 0;
	if ( !$intClearanceSettingID ) {
		$intAutoApproval = $Data->{'SystemConfig'}{'clrDefaultApprovalAction'}
		  || 0;
	}
...
### Once done, it notifies all the levels via email of the current clearance status (ie:Finalised).
	...
	my (
		$intPersonID,   $intClubID,   $intSourceEntityID,
		$Gender,        $DOBAgeGroup, $personType,
		$personSubType, $personLevel, $sport,
		$originLevel,   $ageLevel,    $entityRole,
		$destinationEntityLevel
	) = $query->fetchrow_array();
	$intPersonID       ||= 0;
	$intClubID         ||= 0;
	$intSourceEntityID ||= 0;
	...
	my $genAgeGroup ||=
	  new GenAgeGroup( $Data->{'db'}, $Data->{'Realm'},
		$Data->{'RealmSubType'} );
	...
	my $matrix_ref =
	  getRuleMatrix( $Data, $Defs::ORIGIN_SELF, $destinationEntityLevel,
		$Defs::LEVEL_PERSON, $reg{'entityType'} || '',
		'REGO', \%reg );
...
### PURPOSE: This function is used to create the clearance.  It prepares all of the screens in the create clearance wizard and then passes control to clearanceForm() (a HTMLForm function) to actually display the clearance questions and insert the records into db.
	...
	my $destinationEntityID = getID( $Data->{'clientValues'} )
	  || 0;    #{'clubID'} || 0;
	my $sourceEntityID =
	     $params{'sourceEntityID'}
	  || $params{'d_sourceEntityID'}
	  || 0;
	...
	$params{'member_dob'} = _fix_date( $params{'member_dob'} )
	  if ( check_valid_date( $params{'member_dob'} ) );
	$params{'member_systemdob'} = ''
	  if !check_valid_date( $params{'member_systemdob'} );
	$params{'member_systemdob'} = _fix_date( $params{'member_systemdob'} )
	  if ( check_valid_date( $params{'member_systemdob'} ) );
	...
	### This is the form
	if (    !$personID
		and !$params{'member_surname'}
		and !$params{'member_dob'}
		and !$params{'member_natnum'}
		and !$params{'member_loggedsurname'}
		and !$params{'member_systemsurname'}
		and !$params{'member_systemdob'} )
	{
		$body .= qq[
			<form action="$Data->{'target'}" method="POST">
			<p>]
		  . $lang->txt(
			'Fill in the members National Number, or enter Surname and DOB')
		  . qq[<br></p>
			<table>
			<tr><td><span class="label">]
		  . $lang->txt('Search on a National Number')
		  . qq[:</span></td><td><span class="formw"><input type="text" name="member_natnum" value=""></span></td></tr>
			<tr><td><span class="label">]
		  . $lang->txt('Search on Surname')
		  . qq[:</span></td><td><span class="formw"><input type="text" name="member_surname" value=""></span></td></tr>
			<tr><td><span class="label">]
		  . $lang->txt('Search on Date of Birth (dd/mm/yyyy)')
		  . qq[:</span></td><td><span class="formw"><input type="text" name="member_dob" value=""></span></td></tr>
			</table>
			<input type="submit" name="submit" value="]
		  . $lang->txt('Select Person') . qq[">	
			$hidden
			</form>
		];
	}
	#### end of Form
		...
		!$personID
		and (  $params{'member_surname'}
			or $params{'member_dob'}
			or $params{'member_natnum'}
			or $params{'member_loggedsurname'}
			or
			( $params{'member_systemsurname'} and $params{'member_systemdob'} )
		)
	  )
	{
		my $strWhere = '';
		...
		if ( $params{'member_loggedsurname'} ) {
			$strWhere .=
			  qq[ AND M.strLocalSurname =$tParams{'member_loggedsurname'}];
		}
		if ( $params{'member_systemsurname'} ) {
			$strWhere .=
			  qq[ AND M.strLocalSurname =$tParams{'member_systemsurname'}];
		}
		...
		my ( $sourceEntity, undef, undef ) =
		  getNodeDetails( $db, $Defs::CLUB_LEVEL_CLEARANCE, $Defs::LEVEL_CLUB,
			$sourceEntityID );
		my $txt_RequestCLR = $lang->txt('Request a Transfer');
		$body .= qq[
			<p>]
		  . $lang->txt('Select a person from the club')
		  . qq[ <b>$sourceEntity</b> ]
		  . $lang->txt('in which to Transfer') . qq[</p>
                	<table class="listTable">
				<tr>
					<th>&nbsp;</th>
		...
		while ( my $dref = $query->fetchrow_hashref() ) {
			my $href =
qq[client=$params{'client'}&amp;sourceEntityID=$dref->{'intSourceEntityID'}&amp;a=CL_createnew&amp;member_natnum=$params{'member_natnum'}];
			$href =
qq[client=$params{'client'}&amp;sourceEntityID=$dref->{'intClubID'}&amp;a=CL_createnew&amp;member_natnum=$params{'member_natnum'}]
			  if ( $params{'member_loggedsurname'} );
			$href =
qq[client=$params{'client'}&amp;sourceEntityID=$dref->{'intClubID'}&amp;a=CL_createnew&amp;member_natnum=$params{'member_natnum'}]
			  if ( $params{'member_systemsurname'} );
			...
			{
				$body .=
				  qq[<td><b>] . $lang->txt('TRANSFERRED') . qq[</b></td>];
			}
			else {
				$body .=
qq[<td><a href="$Data->{'target'}?$href&amp;personID=$dref->{intPersonID}">]
				  . $lang->txt('select')
				  . qq[</a></td>];
			}
			$body .= qq[
		...
		Clearance => {
			fields => {
				...
				MemberName => {
					...
					value =>
qq[$memref->{strLocalFirstname} $memref->{strLocalSurname}],
					type     => 'text',
					readonly => 1,
				},
				NatNum => {
			...
			order => [
				qw(MemberName NatNum DOB strState SourceEntity intReasonForClearanceID strReasonForClearance strPersonLevel strAgeLevel strSport)
			],
			options => {
				labelsuffix       => ':',
				hideblank         => 1,
				...
				beforeaddParams   => [
					$Data,     $client,
					$personID, getID( $Data->{'clientValues'} )
				],
				afteraddFunction => \&postClearanceAdd,
				afteraddParams => [ $option, $Data, $Data->{'db'}, $personID ],
				auditFunction  => \&auditLog,
				auditAddParams  => [ $Data, 'Add',    $lang->txt('Transfer') ],
				auditEditParams => [ $Data, 'Update', $lang->txt('Transfer') ],
				stopAfterAction => 1,
				updateOKtext    => qq[
					<div class="OKmsg">]
				  . $lang->txt('Record updated successfully')
				  . qq[</div> <br>
					<a href="$Data->{'target'}?client=$client&amp;a=CL_list">]
				  . $lang->txt('Return to Transfer')
				  . qq[</a>],
				addOKtext => qq[
					<div class="OKmsg">]
				  . $lang->txt('Record added successfully')
				  . qq[</div> <br>
					<a href="$Data->{'target'}?client=$client&amp;a=CL_list">]
				  . $lang->txt('Return to Transfer')
				  . qq[</a>],
	...
	( $resultHTML, undef ) =
	  handleHTMLForm( $FieldDefs{'Clearance'}, undef, $option, '', $db );

	$resultHTML =
	  qq[<div>]
	  . $lang->txt(
		'This person does not have any Transaction information to display')
	  . qq[</div>]
	  if !ref $dref;
...
sub preClearanceAdd {
	...
	if ( $Data->{'SystemConfig'}{'clrNoMoreAdds'} ) {
		my $error =
		    qq[ <div class="warningmsg">]
		  . $lang->txt('Transfers are unable to be added')
		  . qq[</div>];
		return ( 0, $error );
	}
	...
	while ( my $dref = $query->fetchrow_hashref() ) {
		...
		$error_text .= qq[
                	<div class="warningmsg">]
		  . $lang->txt(
'The selected person is already involved in a pending transfer.  Unable to continue until the below transaction is finalised'
		  )
		  . qq[</div>
				<p>
					<b>] . $lang->txt('Date Requested') . qq[:</b> $dref->{AppliedDate}<br>
...
### PURPOSE: This function build's up the starting points between the two clubs then calls getMeetingPoint() to do the grunt work in finding the top node
	...
	if ($found) {
		...
		$qry_insert->execute( $srcLevel, $Defs::CLUB_LEVEL_CLEARANCE,
			$params->{'sourceEntityID'},
			$count, $Defs::DIRECTION_FROM_SOURCE )
		...
		for my $node ( reverse @sourceNodes ) {
			...
			$qry_insert->execute( $node->[1], 3, $node->[0], $count,
				$Defs::DIRECTION_FROM_SOURCE );
		...
		for my $node (@destinationNodes) {
			...
			next
			  if $skip_first == 1
			; ## SKIP FIRST Destination NODE (ie: Its the top one).  IT WAS HANDLED IN SOURCE.
			next if exists $EntitiesUsed{ $node->[0] };
			$qry_insert->execute( $node->[1], 3, $node->[0], $count,
				$Defs::DIRECTION_TO_DESTINATION );
			$EntitiesUsed{ $node->[0] } = 1;
		...
		if ( !exists $EntitiesUsed{ $params->{'destinationEntityID'} } ) {
			$qry_insert->execute( $destLevel, $Defs::CLUB_LEVEL_CLEARANCE,
				$params->{'destinationEntityID'},
				$count, $Defs::DIRECTION_TO_DESTINATION )
			  if $params->{'destinationEntityID'};
			$destinationEntityPathID = $qry_insert->{mysql_insertid} || 0;
		}
...
sub getMeetingPoint {
	...
	my ( $db, $sourceEntityID, $destinationEntityID, $sourceNodes,
		$destinationNodes )
	  = @_;
	my $found = 0;
	...
	my %EntityStructure = ();
	while ( my $dref = $query->fetchrow_hashref() ) {
		$EntityStructure{ $dref->{'intChildID'} }{ $dref->{'intParentLevel'} } =
		  $dref->{'intParentID'};
	}
...
qq[$memref->{strLocalFirstname} $memref->{strLocalSurname}],
				...
				intClearAction => {
					...
					compulsory        => (
						      $option eq 'add'
						  and $Data->{'SystemConfig'}{'Clearances_ClearAction'}
					  ) ? 1 : 0,
				},

			},
			order => [
				qw(dtApplied MemberName DOB strState strSourceEntityName strDestinationEntityName intReasonForClearanceID strReasonForClearance intClearAction)
			],
			options => {
				labelsuffix => ':',
				hideblank   => 1,
				...
				auditFunction => \&auditLog,
				auditAddParams =>
				  [ $Data, 'Add', $lang->txt('Manual Transfer') ],
				auditEditParams =>
				  [ $Data, 'Update', $lang->txt('Manual Transfer') ],
...
### PURPOSE: This function handles the emailing to all the levels of the current status of the clearance.  It contains the text body and subject of the email.
	...
	return
	  if (
		( $Data->{'SystemConfig'}{'clrEmails_addOnly'} and $action ne 'ADDED' )
		and
		( $Data->{'SystemConfig'}{'clrEmails_Denial'} and $action ne 'DENIED' )
		and (   $Data->{'SystemConfig'}{'clrEmails_Reminder'}
			and $action !~ /REMINDER/ )
	  );
	$cID ||= 0;
	my $db = $Data->{'db'};
	...
	return
	  if (  $cref->{SourceSubType}
		and $cref->{DestSubType}
		and $cref->{SourceSubType} == $cref->{DestSubType}
		and $Data->{'SystemConfig'}{'clrNoEmails_sameSubType'} );
	my $email_subject = '';
	###BUILD UP TEXT
	my $additionalInformation = '';
	if (    $Data->{'SystemConfig'}{'clrEmailAdditionalInfo'}
		and $cref->{'strReasonForClearance'} )
	{
		$additionalInformation = $lang->txt('Additional Information')
		  . qq[: $cref->{'strReasonForClearance'}];
	}
...
$additionalInformation
	...
	my ( $whos_turn, undef, undef ) =
	  getNodeDetails( $db, $cref->{intTableType}, $cref->{intTypeID},
		$cref->{intID} );
	my $emailOnlyCurrentLevel = 0;

	my $viewDetails = $lang->txt(
'To view details, please log into the system and click on the List Transfers option'
	);
	$viewDetails = $Data->{'SystemConfig'}{'clrEmail_detailsLink'}
	  if ( $Data->{'SystemConfig'}{'clrEmail_detailsLink'} );

	$emailOnlyCurrentLevel = 1
	  if ( $Data->{'SystemConfig'}{'clr_EmailOnlyCurrentLevel'} );
	if ( $action eq 'CANCELLED' ) {
		$email_body .=
		  $lang->txt('This Transfer has now been cancelled') . $viewDetails;
		$email_subject = $lang->txt('Transfer cancelled- No.')
		  . qq[:$cref->{intClearanceID}- $cref->{MemberName}];
	}
	if ( $action eq 'DENIED' ) {
		$emailOnlyCurrentLevel = 0;
		$email_body .=
		    $lang->txt('This Transfer has been denied at')
		  . qq[ $whos_turn]
		  . $lang->txt('level') . qq[.]
		  . $lang->txt('The Transfer should NOT be requested again');
		$email_body .=
		  $lang->txt('The reason given for the denial is')
		  . qq[: $cref->{DenialCode}]
		  if ( $cref->{DenialCode} );
		$email_subject = $lang->txt('Transfer DENIED- No.')
		  . qq[:$cref->{intClearanceID}- $cref->{MemberName} - DOB - $cref->{dtDOB}];
	}
	if ( $action eq 'REOPEN' ) {
		$emailOnlyCurrentLevel = 0;
		$email_body .=
		  $lang->txt('The above Transfer has now been reopened.') . qq[

] . $lang->txt('Current Level for Approval') . qq[: $whos_turn

$viewDetails ];
		$email_subject = $lang->txt('Transfer reopened- No.')
		  . qq[:$cref->{intClearanceID}- $cref->{MemberName} - DOB - $cref->{dtDOB}];
	}
	if ( $action eq 'ADDED' ) {
...
$viewDetails ];
		$email_subject = $lang->txt('New request for Transfer- No.')
		  . qq[:$cref->{intClearanceID}- $cref->{MemberName} - $cref->{dtDOB}];
...
$viewDetails
		  ...
		  . $lang->txt(
'Be sure to check the Transfer to see when its your turn to approve/deny it'
		  );
		$email_subject = $lang->txt('Transfer Updated- No.')
		  . qq[:$cref->{intClearanceID}- $cref->{MemberName} - $cref->{dtDOB}];
	}
	if ( $action eq 'FINALISED' ) {
		$email_body .=
		  $lang->txt('The above Transfer has been finalised') . $viewDetails;
		$email_subject = $lang->txt('Transfer finalised- No.')
		  . qq[:$cref->{intClearanceID}- $cref->{MemberName} - DOB - $cref->{dtDOB}];
	...
	while ( my $dref = $qry_path->fetchrow_hashref() ) {
		next
		  if $emailOnlyCurrentLevel
			  and $dref->{intClearancePathID} != $cref->{intCurrentPathID};
		my ( undef, undef, $email ) =
		  getNodeDetails( $db, $dref->{intTableType}, $dref->{intTypeID},
			$dref->{intID} );
		my $cs_emails =
		  getServicesContactsEmail( $Data, $dref->{intTypeID}, $dref->{intID},
			$Defs::SC_CONTACTS_CLEARANCES );
...
Content-Transfer-Encoding: 8bit\n\n];
	...
	if ( $mail{To} ) {
		...
		open MAILLOG, ">>$Defs::mail_log_file"
		  or print STDERR "Cannot open MailLog $Defs::mail_log_file\n";
		if ( sendmail %mail ) {
			print MAILLOG ( scalar localtime() )
			  . ":CLR:$cID $action $mail{To}:Sent OK.\n";
			$error = 0;
		}
		else {
			print MAILLOG ( scalar localtime() )
			  . " CLR:$cID $mail{To}:Error sending mail: $Mail::Sendmail::error \n";
		}
		close MAILLOG;
	}
}
...
sub transferDocsForm {
	...
	my $transferdocsfrm = qq[
	    ...
	    <h3> Add Transfer Documents </h3>
         ...
         <br /> 
         <a href="$Data->{'target'}?client=$client&amp;a=CL_list">]
	  . $Data->{'lang'}->txt('Return to Transfer')
	  . qq[</a> 
	    </div>
	];
...
sub showTransferDocuments {
	my ( $Data, $clearanceID ) = @_;
	my $client = setClient( $Data->{'clientValues'} ) || '';

	my $query = qq[
    SELECT *, DATE_FORMAT(dtUploaded,"%d/%m/%Y %H:%i") AS DateAdded_FMT FROM tblUploadedFiles 
INNER JOIN tblDocuments ON tblUploadedFiles.intFileID = tblDocuments.intUploadFileID
WHERE tblDocuments.intClearanceID = ?
    ];

	my $sth = $Data->{'db'}->prepare($query);
	$sth->execute($clearanceID);
	my @rows = ();
	while ( my $dref = $sth->fetchrow_hashref() ) {
		my $url =
"$Defs::base_url/getfile.cgi?client=$client&amp;f=$dref->{'intFileID'}";
		push @rows,
		  {
			ID    => $dref->{'intFileID'}    || 0,
			Title => $dref->{'strTitle'}     || '',
	...
	$sth->finish();
	my $title = $Data->{'lang'}->txt('Supporting Documents');
	my $body  = qq[<br /><div class="sectionheader">$title</div>];
	...
	if ( !@rows ) {
		...
		for my $doc (@rows) {
			...
			my $deleteURL =
"$Data->{'target'}?client=$client&amp;a=DOC_d&amp;dID=$doc->{'ID'}";
			$tbody .= qq[
      <tr $c>
