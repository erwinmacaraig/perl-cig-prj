...
sub listClearances	{
  ...
  while (my $dref = $query->fetchrow_hashref) {
		...
		my %row = ();
		for my $i (qw(MemberName DOB DOB_RAW SourceClubName DestinationClubName updatestatus overallstatus dtApplied dtApplied_RAW createdFrom intClearanceID AlertDate intClearanceYear intClearanceStatus_RAW intClearanceStatus_RAW_filter dtDue dtDue_RAW 
))	{
			$row{$i} = $dref->{$i};
		}
		$row{'id'} = $dref->{'intClearanceID'}.$cnt;
