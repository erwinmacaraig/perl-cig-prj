package Flow_IntTransferReturnBackend;
    ...
    $self->{'ProcessOrder'} = [       
        {
            ...
            'title'  => $lang->txt('International Transfer Return') .' - ' . $lang->txt('Current Club Details'),
        },
        {
            'action' => 'cd',
            'function' => 'display_core_details',
            'label'  => $lang->txt('Personal Details'),
            'title'  => $lang->txt('International Transfer Return') .' - ' . $lang->txt('Check/Update Personal Details'),
            'fieldset'  => 'core',
            #'noRevisit' => 1,
        },
        {
        ...
        },        
        {
            ...
            'title'  => $lang->txt('International Transfer Return') .' - ' . $lang->txt('Check/Update Contact Details'),
        },
        {
            'action' => 'condu',
        ...
        },
        {
            ...
            'title'  => $lang->txt('International Transfer Return') .' - ' . $lang->txt('Confirm Transfer Registration'),
        },
        {
            'action' => 'ru',
        ...
        },
        {
            ...
            'title'  => $lang->txt('International Transfer Return') .' - ' . $lang->txt('Check/Update Documents'),
        },
        {
            'action' => 'du',
        ...
        },
         {
            ...
            'title'  => $lang->txt('International Transfer Return') .' - ' . $lang->txt('Confirm Fee'),
        },
        {
            'action' => 'pu',
        ...
        },
       {
            ...
            'label'  => 'Submit',
            'title'  => 'International Transfer Return - Submitted',
...
sub display_summary { 
    ...
    my %Config = (
        HiddenFields => $self->stringifyCarryField(),
        Target => $self->{'Data'}{'target'},
        ContinueButtonText => scalar(%{$assigneeRef}) ? $self->{'Lang'}->txt('Submit to ' . $initialTaskAssigneeLevel) : $self->{'Lang'}->txt('Mark as Transferred back'),
    );
...
sub display_complete { 
    ...
    my %PageData = (
        ...
        processStatus => 1,
        Content => '',
        Title => $self->{'Data'}{'lang'}->txt("International Transfer Return [_1]", $dref->{'intWFTaskID'} ? 'submitted' : 'has been auto-approved'),
        TextTop => $content,
        TextBottom => '',
        NoContinueButton => 1,
    );
