    ...
    $sql = qq[
      SELECT 
				...
				FROM tblPersonRequest as PRQ
                    ...
                    LEFT JOIN tblTempEntityStructure as TempEntFrom ON (TempEntFrom.intChildID=PRQ.intRequestFromEntityID)
                    LEFT JOIN tblTempEntityStructure as TempEntTo ON (TempEntTo.intChildID=PRQ.intRequestToEntityID)
				WHERE 
                    PRQ.intRealmID = $Data->{'Realm'}
					AND (
                        TempEntFrom.intParentID= $self->{'EntityID'} 
                        OR TempEntTo.intParentID= $self->{'EntityID'}
                    )
					$where_list
    ];
    return ($sql,'');
