...
strItemType
	...
	AND (RI.intItemUsingITCFilter =0
      ...
      #print STDERR "  SELECT RI.intID, RI.intRequired, RI.intUseExistingThisEntity, RI.intUseExistingAnyEntity, COALESCE (LT_D.strString1,D.strDocumentName) as strDocumentName, D.strDocumentFor, COALESCE(LT_D.strNote,D.strDescription) AS strDescription, COALESCE (LT_P.strString1,P.strName) as strProductName, COALESCE(LT_P.strString2,P.strDisplayName) as strProductDisplayName, TP.intTransactionID, RI.intItemUsingActiveFilter, RI.strItemActiveFilterPeriods, RI.intItemActive, RI.intItemUsingPaidProductFilter, RI.strItemActiveFilterPaidProducts, RI.intItemPaidProducts, strItemType FROM tblRegistrationItem as RI LEFT JOIN tblDocumentType as D ON (intDocumentTypeID = RI.intID and strItemType='DOCUMENT') LEFT JOIN tblProducts as P ON (P.intProductID= RI.intID and strItemType='PRODUCT') LEFT JOIN tblLocalTranslations AS LT_P ON ( LT_P.strType = 'PRODUCT' AND LT_P.intID = P.intProductID AND LT_P.strLocale = '$locale' ) LEFT JOIN tblLocalTranslations AS LT_D ON ( LT_D.strType = 'DOCUMENT' AND LT_D.intID = D.intDocumentTypeID AND LT_D.strLocale = '$locale' ) LEFT JOIN tblTransactions as TP ON (TP.intProductID = P.intProductID and TP.intPersonRegistrationID = $Rego_ref->{'intPersonRegistrationID'}) WHERE RI.intRealmID = 1 AND RI.intSubRealmID IN (0, 0) AND RI.strRuleFor = '$ruleFor' AND RI.intOriginLevel IN (99, $originLevel) AND RI.strRegistrationNature IN ('$regNature', '$regNature2') AND RI.strEntityType IN ('', '$Rego_ref->{'strEntityType'}') AND RI.intEntityLevel IN (0, 99, $entityLevel) AND RI.strPersonType IN ('', '$Rego_ref->{'strPersonType'}') AND RI.strPersonLevel IN ('', '$Rego_ref->{'strPersonLevel'}') AND RI.strPersonEntityRole IN ('', '$Rego_ref->{'strPersonEntityRole'}') AND RI.strSport IN ('', '$Rego_ref->{'strSport'}') AND RI.strAgeLevel IN ('', '$Rego_ref->{'strAgeLevel'}') AND RI.strItemType = '$itemType' AND (RI.strISOCountry_IN ='' OR RI.strISOCountry_IN IS NULL OR RI.strISOCountry_IN LIKE CONCAT('%|'$Rego_ref->{'Nationality'}'|%')) AND (RI.strISOCountry_NOTIN ='' OR RI.strISOCountry_NOTIN IS NULL OR RI.strISOCountry_NOTIN NOT LIKE CONCAT('%|'$Rego_ref->{'Nationality'}'|%'))        AND (RI.intFilterFromAge = 0 OR RI.intFilterFromAge <= $Rego_ref->{'currentAge'}) AND (RI.intFilterToAge = 0 OR RI.intFilterToAge >= $Rego_ref->{'currentAge'}) AND (RI.intItemForInternationalTransfer = 0 OR RI.intItemForInternationalTransfer = $Rego_ref->{'InternationalTransfer'}) AND (RI.intItemForInternationalLoan = 0 OR RI.intItemForInternationalLoan = $Rego_ref->{'InternationalLoan'}) AND (RI.intItemUsingITCFilter =0 OR (RI.intItemUsingITCFilter = 1 AND RI.intItemNeededITC = $itc) ) ";
